...
jQuery(document).on("click", "#sync_production_tax", function() {
	process_taxonomy(0);
});

function process_taxonomy(lastRow)
{
	jQuery.ajax({
		url: ajaxurl,
		type: 'POST',
		dataType: 'JSON',
		data: {action: 'process_bulk_taxonony', last_row: lastRow}
	}).done(function(response){
		if(response.status === 'continue') {
			process_taxonomy(response.offset);
